...
export function EventTableSettingsDialog({ open, object, onClose, onPatch }: EventTableSettingsDialogProps) {
  ...
  const content = (() => {
    ...
    if (activeTab === "toolbar") {
      return (
        <div className="event-table-settings-fields event-table-settings-fields--two-col">
          <label className="workbench-field"><span className="workbench-field__label">Toolbar Position</span><select className="workbench-select" value={object.toolbarPosition ?? (object.showToolbar === false ? "hidden" : "top")} onChange={(event) => onPatch({ toolbarPosition: event.target.value as EventTableObject["toolbarPosition"], showToolbar: event.target.value === "hidden" ? false : (object.showToolbar !== false) })}><option value="top">top</option><option value="bottom">bottom</option><option value="hidden">hidden</option></select></label>
          <label className="screen-editor-settings-check"><input type="checkbox" checked={resolved.showToolbar} onChange={(event) => onPatch({ showToolbar: event.target.checked, toolbarPosition: event.target.checked ? (object.toolbarPosition === "hidden" ? "top" : (object.toolbarPosition ?? "top")) : (object.toolbarPosition ?? "top") })} /><span>Show toolbar</span></label>
