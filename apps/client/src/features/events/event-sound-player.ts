  ...
  public preload(soundId: string, sounds: EventSound[]): PlaybackResult {
    ...
    if (!sound) {
      return {
        ok: false,
        reason: "missing_sound",
        message: `Sound '${soundId}' was not found.`,
      };
    }
    ...
    if (!url) {
      return {
        ok: false,
        reason: "missing_url",
        message: `Sound '${sound.name}' does not have a playable file yet.`,
      };
    }
  ...
  public preloadByUrl(cacheKey: string, url: string): PlaybackResult {
    if (!cacheKey.trim()) {
      return {
        ok: false,
        reason: "unknown",
        message: "Sound cache key is required.",
      };
    }
